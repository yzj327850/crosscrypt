...
pub async fn platform_mount(
    ...
    info!("Mounting volume on Linux: {}", device.display());

    let mount_dir = mountpoint.unwrap_or_else(|| {
        format!("/mnt/crosscrypt-{}", uuid::Uuid::new_v4())
    });
...
/// FUSE3 file system implementation
pub struct LinuxFuseFilesystem;

impl LinuxFuseFilesystem {
    pub fn new() -> Self {
        Self
    }
}
